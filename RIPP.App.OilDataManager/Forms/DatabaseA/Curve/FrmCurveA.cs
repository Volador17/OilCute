...
{
    public partial class FrmCurveA : Form
    {
        #region "私有变量"
        ...
        private bool haveOilB = false;//没有B库数据
        /// <summary>
        /// 确定曲线调整是不是需要绘制数据
        /// </summary>
        ...
        /// <param name="oil">一条原油</param>
        /// <param name="typeCode">曲线类型：收率，馏分，渣油</param>
        public FrmCurveA(DatabaseA.FrmOilDataA frmOilDataA, CurveTypeCode typeCode,bool bZH)
        {
            InitializeComponent();
            this.DoubleBuffered = true;
            ...
            this._oil = frmOilDataA.getData();//获取A库数据
            getOilInfoBfromDataBaseB();//从B库中取出对应的曲线数据         
            init(typeCode);//此处调用的函数修改ComboBox的DataSource值，从而激发cmbCurve_SelectedIndexChanged事件，调用处理函数，完成曲线显示和曲线下表格中A、B库数据的填充和显示
            InitStyle(); //表格样式 
            zedGraphEvent();
            if (bZH)
            {
        ...
        public void init(CurveTypeCode typeCode)
        {
            ...
            else if (typeCode == CurveTypeCode.RESIDUE)
            {
                ...
                ECP_WYDic = getECP_WYDatasfromYIELD(this._oilB.curves);
            }

            initXDropList();//X、Y轴下拉菜单的物性绑定，调用函数内部修改cmbCurve.ComboBox的DataSource值，造成SelectedIndex的值发生变化，从而激发cmbCurve_SelectedIndexChanged事件，调用处理函数，完成曲线显示和曲线下表格中A、B库数据的填充和显示
            this.comboBox1.SelectedIndex = 0;

            this.cmbCurve.ComboBox.SelectedIndex = 0;//无用，SelectedIndex从initXDropList()开始变为0
        ...
        private void initXDropList()
        {
            ...
            {
                #region "收率曲线"
                List<CurveParmTypeEntity> XDropList = _curveParmTypeList.Where(o => o.IsX == 11 && o.OilTableTypeID == (int)EnumTableType.Narrow).ToList();//收率曲线的X轴代码IsX=2;
                if (XDropList == null)
                    return;
            ...
            else if (this._typeCode == CurveTypeCode.DISTILLATE)//馏分曲线
            {
                ...
                List<CurveParmTypeEntity> XDropList = DISTILLATECurveParm.Where(o => o.IsX == 21 || o.IsX == 22 || o.IsX == 23).OrderBy(o => o.IsX).ToList();
                if (XDropList == null)
                    return;
            ...
            else if (this._typeCode == CurveTypeCode.RESIDUE)//渣油曲线
            {
                ...
                List<CurveParmTypeEntity> XDropList = RESIDUECurveParm.Where(o => o.IsX != 0).OrderBy(o => o.IsX).ToList();
        ...
        private void initYDropList()
        {
            ...
            if (this._typeCode == CurveTypeCode.YIELD)
            {
                ...
                List<CurveParmTypeEntity> YDropList = this._curveParmTypeList.Where(o =>( o.IsX == 22 || o.IsX == 23) && o.OilTableTypeID == (int)EnumTableType.Narrow).ToList();//收率曲线的IsX == -1表示Y轴代码
            ...
            else if (this._typeCode == CurveTypeCode.RESIDUE)
            {
                ...
                this.cmbCurve.ComboBox.DisplayMember = "Descript";
                this.cmbCurve.ComboBox.ValueMember = "ItemCode";
                this.cmbCurve.ComboBox.DataSource = YDropList;//当DataSource发生改变时，自动激发cmbCurve.ComboBox的SelectedIndexChanged事件，调用相应的处理函数cmbCurve_SelectedIndexChanged（）。只用在运行时跟踪才能找到，编辑状态下是找不到下一步调用函数的
                #endregion
            }           
        }
        ...
        private void cmbCurve_SelectedIndexChanged(object sender, EventArgs e)
        {
            DataBaseACurve.refreshDGVColWidthafterCurveAdjust(ref dataGridView ,DatasBeforAdjust);
            DatabaseA.Curve.DataBaseACurve.getCurrentCurveFromDataGridViewDataBaseB(ref this._oilB, this.dataGridView, this._typeCode, this._curveX, this._curveY);          
            XYSelectedIndexChanged();   //重点函数：曲线作图，曲线下表格数据赋值，显示        
        }
        /// <summary>
        /// XY轴物性改变事件
        /// </summary>
        private void XYSelectedIndexChanged()//重点函数：曲线作图，曲线下表格A、B库数据填充，显示 
        {
            //清空空间计算值
            ...
            if (this._typeCode == CurveTypeCode.YIELD)
            {
                ...
                #region "性质曲线"
                setYIELD();
                if (this.haveOilB && this.haveOilB && currentCurve != null && currentCurve.curveDatas.Count > 0)
                {                   
                    addValuetoLastRowDataGridView(currentCurve.curveDatas);
                }
                else if (!this.haveOilB)//不存在当前曲线
                {
                    if (!FrmCurveAFunction.frmCurveInterpolation( dataGridView, this._typeCode, this._curveX, this._curveY))
                    {
                        //this._HaveSave = true;
                        //this.Close();
                        //this.Dispose();
            ...
            else if (this._typeCode == CurveTypeCode.RESIDUE)
            {
                ...
                if (this.haveOilB)
                {
                    if (currentCurve != null && currentCurve.curveDatas.Count > 0)//存在曲线值
                    {
                        ...
                        if (this._curveX.ItemCode == PartCurveItemCode.WY.ToString())//WY保存
                        {
                            ...
                            _addResidueValueToDataGridViewCurveXRow(ECP_WYDic); //向曲线下表格中B库X轴行数据赋值，对X轴TWY数值进行了小数位数处理，2位小数显示
                            addValuetoResidueLastRowDataGridView(currentCurve.curveDatas);//向曲线下表格中B库Y轴行数据赋值，对Y轴值没有进行小数位数处理；由于上一行对TWY进行了小数位数处理，导致此处调用查找外延的560点的TWY与表格中X轴的TWY显示值的小数位数不同，造成无法显示Y值
                        }
                ...
                else if (!this.haveOilB)//不存在当前曲线//渣油曲线不存在，需要重新计算
                {
                    setRESIDUE();
        ...
        {
            this.toolStripButton9.Enabled = enable;
        ...
        private void setYIELD( )
        {
            ...
            if (this._curveY.ItemCode == "TWY")
            {
                ...
                Dictionary<ZedGraphOilDataEntity, ZedGraphOilDataEntity> ECP_TWYorTVYDictionary = getWTorVLightOilDatas(this._oil.ICP0, Color.Green);
            ...
            {
                Dictionary<ZedGraphOilDataEntity, ZedGraphOilDataEntity> narrowDatas = getDictionary(this._curveX.ItemCode, this._curveY.ItemCode, (int)EnumTableType.Narrow, Color.Red, Color.Blue);
                Dictionary<ZedGraphOilDataEntity, ZedGraphOilDataEntity> wideDatas = getDictionary(PartCurveItemCode.MCP.ToString(), this._curveY.ItemCode, (int)EnumTableType.Wide, Color.Red, Color.Blue);
                Dictionary<ZedGraphOilDataEntity, ZedGraphOilDataEntity> mergeNarrWideDatas = MergeCurveParm(narrowDatas, wideDatas);

                Dictionary<ZedGraphOilDataEntity, ZedGraphOilDataEntity> ECP_TWYorTVYDictionary = getWTorVLightOilDatas(this._oil.ICP0, Color.Green, PartCurveItemCode.TVY);
                Dictionary<ZedGraphOilDataEntity, ZedGraphOilDataEntity> mergeLightNarrWide = MergeCurveParm(mergeNarrWideDatas, ECP_TWYorTVYDictionary);
                bool HaveB_X = true;
                _SetCol(mergeLightNarrWide.Count, HaveB_X);
                _setRow();
        ...
        private void _addResidueValueBeforeTowRowToDataGridView(Dictionary<ZedGraphOilDataEntity, ZedGraphOilDataEntity> mergeResult)
        {
            if (mergeResult != null && mergeResult.Count > 0)
            {
                ...
                {
                    #region "设置第一行"
                    this.dataGridView.Rows[this._ARowXIndex].Cells[this._dataColStart] = new ZedGraphGridOilCell ()
                    {
                        CellValue = whole,
                    ...
                    #region "设置第二行"
                    this.dataGridView.Rows[this._ARowYIndex].Cells[this._dataColStart] = new ZedGraphGridOilCell()
                    {
                ...
                {
                    #region "设置第一行"
                    this.dataGridView.Rows[this._ARowXIndex].Cells[colNum] = new ZedGraphGridOilCell()
                    {
                        CellValue = x,
                    ...
                    #region "设置第二行"
                    this.dataGridView.Rows[this._ARowYIndex].Cells[colNum] = new ZedGraphGridOilCell()
                    {
        ...
        private void _addResidueECPToDataGridView()
        {
            ...
            {
                this.dataGridView.Rows[this._ARowYIndex + 1].Cells[colIndex] = new ZedGraphGridOilCell()
                {
                    Value = ECP,
                    Tag = ECP 
                };
        ...
        private int FindItemCodeValueColIndexfromSpecRow(float itemcodeValue, int rowNum)
        {
            ...
            for (int i = _dataColStart; i < this.dataGridView.ColumnCount; i++)
            {
                ...
                if (xValue != null && xValue.ToString() != string.Empty && !float.IsNaN(float.Parse(xValue.ToString())) && Math.Abs(float.Parse(xValue.ToString()) - itemcodeValue) < 0.01)//20161214修改，解决一键曲线建模后，显示渣油曲线时560C渣油物性值不显示的问题。原编码：...Math.Abs( float.Parse(xValue.ToString())- itemcodeValue)<0.001)
                {
                    index = i;//获取真实的对应列
                    break;
                }
        ...
        private void _addResidueValueToDataGridViewCurveXRow(Dictionary<float, float> ECP_WY)
        {
            ...
            OilTableRowEntity rowWY = OilTableRowBll._OilTableRow.Where(o => o.itemCode == "WY" && o.oilTableTypeID == (int)EnumTableType.Narrow).FirstOrDefault();
            ...
            foreach (var ECP_key in ECP_WY.Keys)
            {
                ...
                {
                    string showValue = oilTool.calDataDecLimit(ECP_WY[ECP_key].ToString(), rowWY.decNumber,rowWY.valDigital);
                    this.dataGridView.Rows[this._curveRowXIndex].Cells[WYCol].Value = showValue;
                    this.dataGridView.Rows[this._curveRowXIndex].Cells[WYCol].Tag = showValue;
                }
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        private static Dictionary<float, float> getECP_TWYDatasfromYIELD(List<CurveEntity> curves)//ECP-TWY
        {
            Dictionary<float, float> ECP_TWYDIC = new Dictionary<float, float>();//存储终切点数据。  

            #region "ECP-TWY判断"
            CurveEntity ECP_TWYCurve = curves.Where(o => o.propertyX == "ECP" && o.propertyY == "TWY").FirstOrDefault();
            if (ECP_TWYCurve == null)
            {
        ...
        /// <param name="itemCode"></param>
        /// <returns></returns>
        public static Dictionary<float, float> getECP_WYDatasfromYIELD(List<CurveEntity> curves)
        {
            Dictionary<float, float> ECP_WYDIC = new Dictionary<float, float>();

            Dictionary<float, float> ECP_TWYDIC = getECP_TWYDatasfromYIELD(curves);//存储终切点数据。           
            ...
            List<float> TWYList = SplineLineInterpolate.spline(X_ECP, Y_TWY, FrmCurveAGlobal.ECPList);//渣油表的八个点的切割
            if (TWYList.Count == 0)
                return ECP_WYDIC;

            for (int ECPIndex = 0; ECPIndex < FrmCurveAGlobal.ECPList.Count; ECPIndex++)
            {
                if (!TWYList[ECPIndex].Equals(float.NaN))
                {
                    float WY = 100 - TWYList[ECPIndex];
                    if (WY != 0)
                        ECP_WYDIC.Add(FrmCurveAGlobal.ECPList[ECPIndex], WY);
                }
            }
        ...
        private void _setRow()
        {
            ...
            //if (tempRowX.itemCode == "MCP")
            //{
            //    row.RowEntity = ECPRow;
            //    row.CreateCells(this.dataGridView, ECPRow.itemName, ECPRow.itemUnit, "应用库");
            //}
            //else
            //{
        ...
        private void _setDataGridViewCurveXRow()
        {
            ...
            {
                this.dataGridView.Rows[row.Index].Cells[colIndex++] = new ZedGraphGridOilCell()
                {
        ...
        private void toolStripButton11_Click(object sender, EventArgs e)
        {
            ...
            if (!FrmCurveAFunction.frmCurveInterpolation(dataGridView, this._typeCode, this._curveX, this._curveY, enumMode.Manu))
            {
                //this._HaveSave = true;
                //this.Close();
                //this.Dispose();
         ...
         private void addValuetoLastRowDataGridView(List<CurveDataEntity> curveDatas)
        {
            ...
            for (int i = 0; i < curveDatas.Count; i++)
            {
                ...
                if (col > 0)
                {
                    this.dataGridView[col, this._lastRowNum].Tag = curveDatas[i].yValue;
                    this.dataGridView[col, this._lastRowNum].Value = curveDatas[i].yValue;
                }
            }
        }
        /// <summary>
        /// 设置渣油曲线的单元格最后一行的值，即Y轴值
        /// </summary>
        /// <param name="keys"></param>
        private void addValuetoResidueLastRowDataGridView(List<CurveDataEntity> curveDatas)
        {
            ...
            for (int i = 0; i < curveDatas.Count; i++)
            {
                ...
                if (colNum != -1)
                {
                    this.dataGridView[colNum, this._curveRowYIndex].Tag = curveDatas[i].yValue;
        ...
        private void toolStripButton12_Click(object sender, EventArgs e)
        {
            ...
            FrmCurveAFunction.frmCurveEpitaxial(dataGridView, this._typeCode, this._curveX, this._curveY,enumMode.Manu);//曲线外延
            drawCurve();
        }
        /// <summary>
        /// 扣除和添加窄馏分
        ...
        /// <returns></returns>
        private Dictionary<ZedGraphOilDataEntity, ZedGraphOilDataEntity> getWTorVLightOilDatas(string ICP0, Color ICP0Color,PartCurveItemCode itemCode = PartCurveItemCode.TWY)
        {
        ...
        private void getOilInfoBfromDataBaseB()
        {
            ...
            this._oilB = OilBll.GetOilByCrudeIndex(crudeIndex);//由原油编号取出B库数据
        ...
        public void SavefrmCurveA()
        {
            try
            {
                ...
                if (this._oilB.ID < 0)//保存当前原油B
                {
                    ...
                    CurveEntity TVYCurve = this._oilB.curves.Where(o => o.propertyY == "TVY").FirstOrDefault();
                    OilTableRowEntity tempRowTVY = OilTableRowBll._OilTableRow.Where(o => o.itemCode == "TVY").FirstOrDefault();
                    ...
                    if (TVYCurve == null)
                    {
                        ...
                        OilDataEntity D20Data = this._oil.OilDatas.Where(o => o.OilTableRow.itemCode == "D20" && o.OilTableTypeID == (int)EnumTableType.Whole).FirstOrDefault();
                        CurveEntity TWYCurve = this._oilB.curves.Where(o => o.propertyY == "TWY" && o.curveTypeID == (int)CurveTypeCode.YIELD).FirstOrDefault();
                        CurveEntity D20Curve = this._oilB.curves.Where(o => o.propertyY == "D20" && o.curveTypeID == (int)CurveTypeCode.DISTILLATE).FirstOrDefault();
                        ...
                        {   //注意此处的曲线没有ID
                            ...
                            TVYCurve.curveTypeID = (int)EnumTableType.Narrow;
                            TVYCurve.oilInfoID = this._oilB.ID;
                            ...
                            this._oilB.curves.Add(TVYCurve);
                        }                      
                    }
                    ...
                    for (int curveIndex = 0; curveIndex < this._oilB.curves.Count; curveIndex++)
                    {
                        ...
                        //if (this._oilB.curves[curveIndex].propertyX.Equals("MCP"))
                        //    newCurve.propertyX = "ECP";
                        //else 
        ...
        private void dataGridView_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            PointPairList Blist = this.zedGraph1.ZedExpand.PointList;       //获取B库点的集合.???error,PointList不包括560点的值
        ...
        private void dataGridView_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            this.dataGridView[e.ColumnIndex, e.RowIndex].Tag = this.dataGridView[e.ColumnIndex, e.RowIndex].Value;   //根据行列号获取单元格的值                     
            drawCurve();   //重新生成曲线
        }
        ...
        private void refreshCurve()
        {
            ...
            for (int col = this._dataColStart; col < colLength; col++)
            {
                ...
                this.dataGridView.Rows[this._curveRowYIndex].Cells[col].Tag = string.Empty;
            }
        ...
        private void setYIELD(OilInfoEntity oilInfoA )
        {
            ...
            if (this._curveY.ItemCode == "TWY")
            {
                ...
                Dictionary<ZedGraphOilDataEntity, ZedGraphOilDataEntity> mergeNarrWideDatas = MergeCurveParm(narrowDatas, wideDatas);
                Dictionary<ZedGraphOilDataEntity, ZedGraphOilDataEntity> ECP_TWYorTVYDictionary = getWTorVLightOilDatas(oilInfoA.ICP0, Color.Green);
                Dictionary<ZedGraphOilDataEntity, ZedGraphOilDataEntity> mergeLightNarrWide = MergeCurveParm(mergeNarrWideDatas, ECP_TWYorTVYDictionary);
                bool HaveB_X = true;
            ...
            {
                Dictionary<ZedGraphOilDataEntity, ZedGraphOilDataEntity> narrowDatas = DataBaseACurve.getDictionary(oilInfoA, this._typeCode, this._curveX.ItemCode, this._curveY.ItemCode, EnumTableType.Narrow, Color.Red, Color.Blue);
                Dictionary<ZedGraphOilDataEntity, ZedGraphOilDataEntity> ECP_TWYorTVYDictionary = getWTorVLightOilDatas(oilInfoA.ICP0, Color.Green,PartCurveItemCode.TVY);
        ...
        private void setRESIDUE(OilInfoEntity oilInfoA )
        {
            ...
            if (this._curveX.ItemCode == PartCurveItemCode.WY.ToString())//WY保存
            {
                ...
                _SetResidueCol(residueA_XYDatas.Count, true);//设置渣油曲线列头      
                _setResidueRow();
                _addResidueValueBeforeTowRowToDataGridView(residueA_XYDatas);//向渣油曲线图下数据表中第1，2行赋值
                _addResidueECPToDataGridView();//向渣油曲线图下数据表中第3行“初切点”赋值
                _addResidueValueToDataGridViewCurveXRow(ECP_WYDic ); //向B库赋值
            }
            else if (this._curveX.ItemCode == PartCurveItemCode.VY.ToString())//TVY显示
            {
        ...
        private void toolStripButton9_Click(object sender, EventArgs e)
        {
            ...
            {
                DataBaseACurve.refreshDGVColWidthafterCurveAdjust(ref dataGridView, DatasBeforAdjust);
                refreshCurve();//刷新曲线，初始化

                if (!FrmCurveAFunction.frmCurveInterpolation(dataGridView, this._typeCode, this._curveX, this._curveY))//曲线内插
                {
                    //this._HaveSave = true;
                    //this.Close();
                    //this.Dispose();
                ...
                {
                    FrmCurveAFunction.frmCurveAutoEpitaxial(dataGridView, this._typeCode, this._curveX, this._curveY);//曲线外延，调用入口
                    drawCurve();
                }
            }
            catch (Exception ex)
