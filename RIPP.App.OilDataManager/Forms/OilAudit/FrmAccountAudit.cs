...
namespace RIPP.App.OilDataManager.Forms.OilAudit
{
    ...
    /// 核算审查窗体
    /// </summary>
    public partial class FrmAccountAudit : Form
    {
        #region "私有函数"
        ...
        public FrmAccountAudit(OilInfoEntity oil)
        {
            InitializeComponent();
            this.groupBox7.MouseDown += groupBox7_MouseDown;
            this.groupBox7.MouseUp += groupBox7_MouseUp;
        ...
        /// </summary>
        /// <param name="oil"></param>
        public FrmAccountAudit(GridOilViewA wholeGridOil, GridOilViewA narrowGridOil, GridOilViewA wideGridOil, GridOilViewA residueGridOil)
        {
            InitializeComponent();
            this.groupBox7.MouseDown += groupBox7_MouseDown;
            this.groupBox7.MouseUp += groupBox7_MouseUp;
        ...
        private void btnAcc_Click(object sender, EventArgs e)
        {
            ...
            {
                fillDataToDgv(this.dgvShow, colName, this._Dic[colName], "Lab");
                fillDataToDgv(this.dgvShow, colName, this._Dic[colName], "Cal");
            }               
        }
        ...
        private void setDgv(DataGridView targetDgv,ListView lvRows, Dictionary<string, List<CutMothedEntity>> dicCols)
        {
            ...
            {
                targetDgv.Columns.Add(new DataGridViewTextBoxColumn() { Name = key + "[原始值]", HeaderText = key + "[原始值]", AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells, MinimumWidth = 120 });
                targetDgv.Columns.Add(new DataGridViewTextBoxColumn() { Name = key, HeaderText = key, AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells,MinimumWidth = 80 });
            }
        ...
        private void fillDataToDgv(DataGridView targetDgv,string colName, List<CutMothedEntity> cutMothedList,string Type = "Lab")
        {
            #region "核算性质检查"
            ...
            List<OilDataEntity> NarrowICPList = this._narrowGridOil.GetDataByRowItemCode("ICP").Where(o => o.calShowData != string.Empty).ToList();
            List<OilDataEntity> NarrowECPList = this._narrowGridOil.GetDataByRowItemCode("ECP").Where(o => o.calShowData != string.Empty).ToList();

            List<OilDataEntity> WideList = this._wideGridOil.GetAllData().Where(o => o.calShowData != string.Empty).ToList();
            List<OilDataEntity> WideICPList = this._wideGridOil.GetDataByRowItemCode("ICP").Where(o => o.calShowData != string.Empty).ToList();
            List<OilDataEntity> WideECPList = this._wideGridOil.GetDataByRowItemCode("ECP").Where(o => o.calShowData != string.Empty).ToList();

            List<OilDataEntity> ResList = this._residueGridOil.GetAllData().Where(o => o.calShowData != string.Empty).ToList();
            List<OilDataEntity> ResidueICPList = this._residueGridOil.GetDataByRowItemCode("ICP").Where(o => o.calShowData != string.Empty).ToList();
            List<OilDataEntity> wholeList = this._wholeGridOil.GetAllData();
            ...
            {
                #region            
                int result = checkCutMothedList(cutMothedList, colName, NarrowICPList, ResidueICPList);
                ...
                else if (result == 0)//原油
                {
                    for (int rowIndex = 0; rowIndex < this.dgvShow.Rows.Count; rowIndex++)
                    {
                        ...
                        var tempWhole = wholeList.Where(o => o.OilTableRow.itemCode == itemCode).FirstOrDefault();
                        if (tempWhole != null)
                            this.dgvShow.Rows[rowIndex].Cells[colName + "[原始值]"].Value = tempWhole.calShowData;
                    }
                }
                else if (result == 1)//宽馏分
                {
                    List<OilDataEntity> oilDataICPList = null;
                    #region
                    if (cutMothedList.FirstOrDefault().strICP == string.Empty)
                    {
                        var ICP0 = this._narrowGridOil._datas.Where(t => t.OilTableRow.itemCode == "ICP" && t.OilTableCol.ColumnIndex == 1).FirstOrDefault();
                        if (ICP0 == null)
                        {
                            MessageBox.Show(colName + "[原始值]方案找不到初切点为" + cutMothedList.FirstOrDefault().strICP + "对应的馏分!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            return ;
                        }

                        oilDataICPList = WideICPList.Where(o => o.calShowData == ICP0.calShowData).ToList();
                    }
                    else//宽馏分
                    {
                        oilDataICPList = WideICPList.Where(o => o.calShowData == cutMothedList.FirstOrDefault().strICP).ToList();
                    }
                    #endregion

                    if (oilDataICPList == null)
                    {
                        MessageBox.Show(colName + "[原始值]方案找不到初切点为" + cutMothedList.FirstOrDefault().strICP + "对应的馏分!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }

                    OilDataEntity oilDataECP = null;
                    #region
                    foreach (var oilDataICP in oilDataICPList)
                    {
                        oilDataECP = WideECPList.Where(o => o.OilTableCol.colCode == oilDataICP.OilTableCol.colCode && o.calShowData == cutMothedList.LastOrDefault().strECP).FirstOrDefault();
                    ...
                    if (oilDataECP == null)
                    {
                        MessageBox.Show(colName + "[原始值]方案找不到终切点为" + cutMothedList.FirstOrDefault().strECP + "对应的馏分!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
                    ...
                    {
                        string itemCode = this.dgvShow.Rows[rowIndex].Cells["itemName"].Tag.ToString();
                        var tempWide= WideList.Where(o => o.OilTableRow.itemCode == itemCode && o.OilTableCol.colCode == oilDataECP.OilTableCol.colCode).FirstOrDefault();
                        if (tempWide != null)
                            this.dgvShow.Rows[rowIndex].Cells[colName + "[原始值]"].Value = tempWide.calShowData;
                    }
                }
                else if (result == 2)//渣油
                {
                    var oilDataResICP = ResidueICPList.Where(o => o.calShowData == cutMothedList.FirstOrDefault().strICP).FirstOrDefault();
                    if (oilDataResICP == null)
                        MessageBox.Show(colName + "[原始值]方案找不到初切点为" + cutMothedList.FirstOrDefault().strICP + "对应的馏分!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);

                    for (int rowIndex = 0; rowIndex < this.dgvShow.Rows.Count; rowIndex++)
                    { 
                        string itemCode = this.dgvShow.Rows[rowIndex].Cells["itemName"].Tag.ToString();
                        var tempRes = ResList.Where(o => o.OilTableRow.itemCode == itemCode && o.OilTableCol.colCode == oilDataResICP.OilTableCol.colCode).FirstOrDefault();

                        if (tempRes != null)
                            this.dgvShow.Rows[rowIndex].Cells[colName + "[原始值]"].Value = tempRes.calShowData;
                    }
                }
                #endregion
            }
        ...
        /// <param name="cutMothedList"></param>
        /// <returns></returns>
        private int checkCutMothedList(List<CutMothedEntity> cutMothedList, string colName, List<OilDataEntity> NarrowICPList, List<OilDataEntity> ResidueICPList)
        {
            if (cutMothedList.Count == 0)
                return -1;           
       
            if (cutMothedList.FirstOrDefault().strICP == "" && cutMothedList.LastOrDefault().strECP == "")//原油
                return 0;
            else if (cutMothedList.FirstOrDefault().strICP != "" && cutMothedList.LastOrDefault().strECP == "")
            {
                #region 

                var resICP = ResidueICPList.Where(t => t.calShowData == cutMothedList.FirstOrDefault().strICP).FirstOrDefault();

                if (resICP != null)
                    return 2;//渣油
                else
                {
                    var narICP = NarrowICPList.Where(t => t.calShowData == cutMothedList.FirstOrDefault().strICP).FirstOrDefault();

                    if (narICP != null && narICP.ColumnIndex == 0)
                        return 0;   //宽馏分   
                    else
                        return -1;
                }
        ...
        {
            if (MessageBox.Show("是否删除所有核算方案?", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == System.Windows.Forms.DialogResult.OK)
            {
                if (this.tabControlEx1.TabCount > 0)
        ...
        {
            if (MessageBox.Show("是否删除当前方案?", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == System.Windows.Forms.DialogResult.OK)
            {
                if (this.tabControlEx1.TabCount > 0)
                    this.tabControlEx1.TabPages.Remove(this.tabControlEx1.SelectedTab);
