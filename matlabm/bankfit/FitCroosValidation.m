function [allTQ,allSQ,allRatio,allIdx,allFit] = FitCroosValidation(x,wind,waveIdx)
% 库光谱拟合法交互验证
...
% TQ，1*校正集样品数目
	...
	for ii=1:n
		...
		xx=x;    
		xx(:,ii)=[]; 
		[TQ, SQ, ratio, idx, fit ] = FitPredictor(xx,y,wind,waveIdx,1);
		idx(idx>=ii) = idx(idx>=ii) + 1;
		allTQ(ii) = TQ;
		allSQ(ii) = SQ; 
