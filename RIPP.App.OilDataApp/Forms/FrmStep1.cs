...
namespace RIPP.App.OilDataApp.Forms
{
    public partial class FrmStep1 : Form 
    {
        ...
        public FrmStep1(FrmMain frmMain)
        {
            InitializeComponent();
        ...
        private void GridListSourceBind( )
        {
            ...
            this.toolStripStatusLabel1.Text = "共有" + oilInfo.Count.ToString() + "条信息满足条件。";
        }
        /// <summary>
        /// 重新刷新数据
        /// </summary>
        ...
        private void GridListSelectBind()
        {
            ...
            for (int i = 0; i < this._frmMain.CutOilRates.Count; i++)//选择了几条原油
            {
        ...
        /// <summary>
        /// 确定按钮，把选中原油添加到主窗体原油和比例实体变量中
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnNextStep_Click(object sender, EventArgs e)
        {
            this._frmMain.CutOilRates.Clear();
            foreach (DataGridViewRow row in gridListSelect.Rows)
            {
                ...
                this._frmMain.CutOilRates.Add(cutOilRate);
            }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void gridListSource_DoubleClick(object sender, EventArgs e)
        {           
            int OilBID = this.gridList.CurrentRow != null ? int.Parse(this.gridList.CurrentRow.Cells["ID"].Value.ToString()) : -1;

            OilInfoBEntity oilB = OilBll.GetOilBByID(OilBID);
            if (oilB == null)
                return;
            if (_isOilOpening)
                return;
            ...
            try
            {
                FrmOilDataB child = (FrmOilDataB)this._frmMain.GetChildFrm(oilB.crudeIndex + "B");
                if (child == null)
                {
                    FrmOilDataB form = new FrmOilDataB(oilB);
                    //form.MdiParent = this._frmMain;
                    form.Show();
                }
                else
                {
