...
namespace ZedGraph
{
	...
	#region Private Fields
        ...
        /// 线的名称显示位置
        /// </summary>
        private enumLineNameLocation _lineNameLocation = enumLineNameLocation.None;
	#endregion
        /// <summary>
        /// 线的名称显示位置
        /// </summary>
        public enumLineNameLocation LineNameLocation
        {
            get { return this._lineNameLocation; }
            set { this._lineNameLocation = value; }
        }
	#region Defaults
	...
	#region Rendering Methods
		...
		{
            #region
            if (this._lineNameLocation == enumLineNameLocation.On)
            {
                foreach (CurveItem mycurve in _curveList)
                {
                    ...
                    if (points.Count >= 2)
                    {
                        ...
                        if (points.Count % 2 == 1)
                        {
                            text.Location.X = points[mid].X;
                            text.Location.Y = points[mid].Y;
                        }
                        else if (points.Count % 2 == 0)
                        {
                            text.Location.X = (points[mid].X + points[mid - 1].X) / 2;
                            text.Location.Y = (points[mid].Y + points[mid - 1].Y) / 2;
                        }
                    }
                    else
                    {
                        text.Location.X = points[0].X;
                        text.Location.Y = points[0].Y;
                    }
