...
namespace ZedGraph
{
	...
	{
		private ZedGraphControlEventExpand zedExpand = new ZedGraphControlEventExpand();
		public ZedGraphControlEventExpand ZedExpand
		{
			get {
				zedExpand.zg1 = this;//在调用zedExpand的时候初始化zg1变量
				return zedExpand; }
		}
	...
	#region Mouse Events
		...
		protected void ZedGraphControl_MouseDown( object sender, MouseEventArgs e )
		{
			...
			zedExpand.Down_Expand(this, mousePt,false,ZedExpand.IsMoving);
			if ( pane != null &&
		...
		{
			zedExpand.Up_Expand(this,new Point(e.X,e.Y),false);
			...
			if ( _masterPane != null && _dragPane != null )
			{
				...
				if ( _isZooming && !zedExpand.ZoomChoose && zedExpand.Zoom)
		...
		protected void ZedGraphControl_MouseMove( object sender, MouseEventArgs e )
		{
			if ( _masterPane != null )
			{
				...
				zedExpand.Move_Expand(this,mousePt,false,ZedExpand.IsMoving);
	...
	#region Mouse Wheel Zoom Events
		...
		protected void ZedGraphControl_MouseWheel( object sender, MouseEventArgs e )
		{
			...
			{
				MouseExpand(new PointF(e.X,e.Y),e.Delta);
			}
		}
		...
		public void MouseExpand(PointF centerPoint,double delta)
		{
			GraphPane pane = this.MasterPane.FindChartRect(centerPoint);
	...
	#region Zoom Events
		...
		private void HandleZoomDrag( Point mousePt )
		{
			...
			if (ZedExpand.ZoomChoose && !ZedExpand.IsMoving)
			{
				ControlPaint.FillReversibleRectangle(rect, Color.FromArgb(150, 0, 0, 255));
			}
		...
		private Rectangle CalcScreenRect( Point mousePt1, Point mousePt2 )
		{
			...
			if ( _isZooming )
			{
				...
				if ( _isEnableVZoom && ZedExpand.ZoomChoose )
				{
					rect.Y = chartPt.Y;
